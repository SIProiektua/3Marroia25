...
public class FlightBooking extends JFrame {
	...
	private List<ConcreteFlight> savercollect;
		...
		lookforFlights.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while (flights.hasNext()) {
					cf = flights.next();
					System.out.println(cf.getConcreteFlightCode());
					try {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (bussinesTicket.isSelected()) {
					...
					if (num > 0) {
						...
						cff = businessLogic.changeConcreteFlight(selectedConcreteFlight.getConcreteFlightCode(), "business", num - 1);
					} else
						error = true;
				}
				if (firstTicket.isSelected()) {
					...
					if (num > 0) {
						...
						cff = businessLogic.changeConcreteFlight(selectedConcreteFlight.getConcreteFlightCode(), "first", num - 1);
					} else
						error = true;
				}
				if (touristTicket.isSelected()) {
					...
					if (num > 0) {
						...
						cff = businessLogic.changeConcreteFlight(selectedConcreteFlight.getConcreteFlightCode(), "tourist", num - 1);
					} else
						error = true;
				}
				if (error) {
